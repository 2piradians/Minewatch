...
public class GuiHeroSelect extends GuiScreen implements IGuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		case MAIN:
			...
			GlStateManager.scale(scale, scale, 1);
			EnumHero.displayPortrait(this.getSelectedHero(), this.width/2/scale-128, this.height/2/scale, this.getSelectedHero() != SetManager.getWornSet(mc.thePlayer), false);
			GlStateManager.scale(1/scale, 1/scale, 1);
			this.drawCenteredString(mc.fontRendererObj, TextFormatting.AQUA+""+TextFormatting.ITALIC+mc.thePlayer.getName().toUpperCase(), (int) (this.width/2), (int) (this.height*0.67f), 0xFFFFFF);
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		switch (button.id) {
		case 100: 
			Minewatch.network.sendToServer(new CPacketSimple(18, mc.thePlayer, this.getSelectedHero().ordinal(), 0, 0));
			Minecraft.getMinecraft().displayGuiScreen(null);
			ModSoundEvents.GUI_CHOOSE_HERO.playFollowingSound(mc.thePlayer, 1, 1, false);
			break;
		case 101:
	...
	public EnumHero getSelectedHero() {
		if (this.selectedHero == null) {
			if (SetManager.getWornSet(mc.thePlayer) != null) // worn hero
				this.setSelectedHero(SetManager.getWornSet(mc.thePlayer));
			else // random hero
				this.setSelectedHero(EnumHero.values()[mc.theWorld.rand.nextInt(EnumHero.values().length)]);
		}
	...
	public void setSelectedHero(@Nullable EnumHero hero) {
		if (this.selectedHero != hero) {
			...
			if (hero != null) {
				...
				guiPlayer.skin = hero.getSkin(Minecraft.getMinecraft().thePlayer.getPersistentID());
				if (this.selectedHero != null)
					ModSoundEvents.GUI_SELECT_HERO.playFollowingSound(mc.thePlayer, 1, 1, false);
			}
	...
	public void onGuiClosed() {
		ModSoundEvents.GUI_CLOSE.playFollowingSound(mc.thePlayer, 1, 1, false);
	}
