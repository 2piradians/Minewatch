...
/**Mostly copied from ServerListEntryNormal*/
...
public class ServerListEntry extends ServerListEntryNormal
{
    ...
    protected ServerListEntry(GuiMultiplayer owner, ServerData serverIn)
    {
    	super(owner, serverIn);
        this.owner = owner;
        ...
        if (!this.server.pinged)
        {
            ...
            EXECUTOR.submit(new Runnable()
            {
                public void run()
                {
                    try
                    {
                        ServerListEntry.this.owner.getOldServerPinger().ping(ServerListEntry.this.server);
                    }
                    catch (UnknownHostException var2)
                    {
                        ServerListEntry.this.server.pingToServer = -1L;
                        ServerListEntry.this.server.serverMOTD = TextFormatting.DARK_RED + "Can\'t resolve hostname";
                    }
                    catch (Exception var3)
                    {
                        ServerListEntry.this.server.pingToServer = -1L;
                        ServerListEntry.this.server.serverMOTD = TextFormatting.DARK_RED + "Can\'t connect to server.";
                    }
                }
            });
        }
