...
public class GuiTeamBlock extends GuiTeamSelector implements IGuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (te.isInvalid() && mc.theWorld.getTileEntity(te.getPos()) instanceof TileEntityTeam)
			te = (TileEntityTeam) mc.theWorld.getTileEntity(te.getPos());
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		switch (button.id) {
		case 1: // activate
			Minewatch.network.sendToServer(new CPacketSimple(15, true, mc.thePlayer, te.getPos().getX(), te.getPos().getY(), te.getPos().getZ()));
			break;
		case 2: // deactivate
			Minewatch.network.sendToServer(new CPacketSimple(15, false, mc.thePlayer, te.getPos().getX(), te.getPos().getY(), te.getPos().getZ()));
			break;
		case 3: // ?
	...
	public void setSelectedTeam(@Nullable Team team, boolean sendPacket) {
		if ((team != null && !team.isSameTeam(selectedTeam)) || 
			...
			if (sendPacket)
				Minewatch.network.sendToServer(new CPacketSimple(13, team == null ? null : team.getRegisteredName(), mc.thePlayer, te.getPos().getX(), te.getPos().getY(), te.getPos().getZ()));
			selectedTeam = team;
		}
	}
	...
	public void onGuiClosed() {
		...
		if (!nameField.getText().equals(te.getName()) && te.isValidName(nameField.getText())) {
			...
			Minewatch.network.sendToServer(new CPacketSimple(14, nameField.getText(), mc.thePlayer, te.getPos().getX(), te.getPos().getY(), te.getPos().getZ()));
		}
	}
