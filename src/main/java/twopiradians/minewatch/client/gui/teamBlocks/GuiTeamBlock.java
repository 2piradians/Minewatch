...
public class GuiTeamBlock extends GuiTeamSelector implements IGuiScreen {
	...
	public void initGui() {
		...
		this.buttonList.add(new GuiButtonBase(-2, guiLeft+X_SIZE/2-120/2, offsetY+guiTop+58, 120, 20, format+Minewatch.translate("gui.team_block.button.options"), this).setVisiblePredicate(gui->gui.getCurrentScreen() == Screen.MAIN).setNoSound().setCustomRender(Render.TEXT)); 
		this.buttonList.add(new GuiButtonBase(1, guiLeft+X_SIZE/2-60/2, offsetY+guiTop+77, 60, 20, TextFormatting.GRAY+Minewatch.translate("gui.team_block.button.inactive"), this).setVisiblePredicate(gui->gui.getCurrentScreen() == Screen.MAIN && !te.isActivated()).setColor(new Color(255, 50, 50)));
		this.buttonList.add(new GuiButtonBase(2, guiLeft+X_SIZE/2-60/2, offsetY+guiTop+77, 60, 20, TextFormatting.BOLD+Minewatch.translate("gui.team_block.button.active"), this).setVisiblePredicate(gui->gui.getCurrentScreen() == Screen.MAIN && te.isActivated()).setColor(new Color(50, 255, 50)));
