...
public class GuiTeamSpawn extends GuiTeamBlock {
	...
	public void drawMainScreen(int mouseX, int mouseY, float partialTicks) {
		...
		case MAIN:
			this.drawCenteredString(mc.fontRendererObj, String.valueOf(((TileEntityTeamSpawn) te).getSpawnRadius()), guiLeft+X_SIZE/2, offsetY+guiTop+24, 0xFFFFFF);
			break;
		case QUESTION_MARK:
	...
	protected List<String> getStatusText() {
		...
		if (this.getSelectedTeam() != null)
			team = this.getSelectedTeam().getChatFormat()+getTeamName(this.getSelectedTeam());
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		case 6: // +
			if (((TileEntityTeamSpawn) te).getSpawnRadius() < TileEntityTeamSpawn.MAX_SPAWN_RADIUS)
				Minewatch.network.sendToServer(new CPacketSimple(16, true, mc.thePlayer, te.getPos().getX(), te.getPos().getY(), te.getPos().getZ()));
			break;
		case 7: // -
			if (((TileEntityTeamSpawn) te).getSpawnRadius() > 0)
				Minewatch.network.sendToServer(new CPacketSimple(16, false, mc.thePlayer, te.getPos().getX(), te.getPos().getY(), te.getPos().getZ()));
			break;
		}
	}	
