...
public class GuiTeamStick extends GuiTeamSelector {
	...
	public void initGui() {
		...
		this.buttonList.add(new GuiButtonURL(8, guiLeft+X_SIZE/2-70, guiTop+30, 140, 20, TextFormatting.BLUE+""+TextFormatting.UNDERLINE+"How to use the Team Stick", Minewatch.MAP_TOOLS_VIDEO_URL, this)); 
		...
		teamNameField = new GuiTextField(0, mc.fontRenderer, guiLeft+7, guiTop+73, 104, 14);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (this.teamWaitingFor != null) {
			Team team = mc.world.getScoreboard().getTeam(this.teamWaitingFor);
			if (team != null) {
				teams.add(team);
				teams.sort(new Comparator<Team>() {
					@Override
		...
		if (this.currentScreen == Screen.MAIN)
			teamNameField.x = guiLeft+7;
		else
			teamNameField.x = guiLeft+X_SIZE/2-teamNameField.width/2;

		// QUESTION_MARK info screen
		if (this.currentScreen == Screen.QUESTION_MARK) {
			mc.fontRenderer.drawString(TextFormatting.DARK_GRAY.toString()+TextFormatting.ITALIC+
		...
		if (getSelectedTeam() != null) {
			...
			if (this.currentScreen == Screen.INFO) {
				...
				int y = guiTop+8;
				for (String s : mc.fontRenderer.listFormattedStringToWidth(text, X_SIZE-16)) {
					mc.fontRenderer.drawString(s, guiLeft+8, y, 0xFFFFFF);
			...
			this.drawCenteredString(mc.fontRenderer, TextFormatting.GRAY+""+TextFormatting.ITALIC+"No teams created", guiLeft+X_SIZE/2, guiTop+Y_SIZE-3-52-mc.fontRenderer.FONT_HEIGHT/2, 0xFFFFFF);
		else if (this.currentScreen == Screen.MAIN) {
			String text = TextFormatting.DARK_GRAY+""+TextFormatting.ITALIC+"or select a team below...";
			mc.fontRenderer.drawString(text, guiLeft+X_SIZE/2-mc.fontRenderer.getStringWidth(text)/2, guiTop+100, 0xFFFFFF);
		}

		if ((this.getSelectedTeam() != null && this.currentScreen == Screen.EDIT_TEAM) || this.currentScreen == Screen.CREATE_TEAM) {
			TextFormatting format = this.getSelectedTeam() == null ? this.selectedColor : this.getSelectedTeam().getColor();
			this.drawCenteredString(mc.fontRenderer, format+""+TextFormatting.UNDERLINE+"Team Color", guiLeft+X_SIZE/2, guiTop+8, 0xFFFFFF);
			this.drawCenteredString(mc.fontRenderer, format+""+TextFormatting.UNDERLINE+"Team Name", guiLeft+X_SIZE/2, guiTop+58, 0xFFFFFF);
		...
		if (this.getSelectedTeam() != null && this.currentScreen == Screen.INFO) {
			...
			this.drawCenteredString(mc.fontRenderer, title, guiLeft-3-50, guiTop+11, 0xFFFFFF);
		}
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		if (button instanceof GuiButtonTeamColor) {
			...
			else if (this.getSelectedTeam() != null && this.getSelectedTeam().getColor() != null && 
					this.getSelectedTeam().getColor().getColorIndex() != button.id) {
				Minewatch.network.sendToServer(new CPacketSimple(7, this.getSelectedTeam().getName(), mc.player, button.id, 0, 0));
			}
		}
		// add to team
			...
			Minewatch.network.sendToServer(new CPacketSimple(6, false, mc.player, 0, 0, 0, entity, null, this.getSelectedTeam().getName()));
	...
	public void confirmClicked(boolean result, int id) {
		...
		if (id == 906) {
			...
			if (result && this.getSelectedTeam() != null) {
				Minewatch.network.sendToServer(new CPacketSimple(8, this.getSelectedTeam().getName(), mc.player));
	...
	protected void mouseClicked(int x, int y, int button) throws IOException {
		...
		if (button == 1 && x >= teamNameField.x && x < teamNameField.x + teamNameField.width && y >= teamNameField.y && y < teamNameField.y + teamNameField.height) { 
	...
	public void setSelectedTeam(@Nullable Team team, boolean sendPacket) {
		if ((team != null && !team.isSameTeam(selectedTeam)) || 
			...
			if (sendPacket)
				Minewatch.network.sendToServer(new CPacketSimple(5, team == null ? null : team.getName(), mc.player));
			selectedTeam = team;
