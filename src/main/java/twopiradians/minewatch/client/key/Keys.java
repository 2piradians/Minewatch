	...
	public enum KeyBind {
		...
		public void setCooldown(EntityLivingBase player, int cooldown, boolean silent) {
			if (player != null) {
				...
				TickHandler.register(player.worldObj.isRemote, COOLDOWNS.setEntity(player).setTicks(Math.max(3, (int) (cooldown * Config.abilityCooldownMultiplier))));
				if (player.worldObj.isRemote && (silent || cooldown * Config.abilityCooldownMultiplier < 20)) {
		...
		public void setKeyDown(EntityLivingBase entity, boolean isKeyDown) {
			this.setKeyDown(entity == null ? null : entity.getPersistentID(), isKeyDown, entity != null ? entity.worldObj.isRemote : true);
		}
