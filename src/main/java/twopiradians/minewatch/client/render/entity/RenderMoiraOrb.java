...
public class RenderMoiraOrb extends RenderSimple<EntityMoiraOrb> {
	...
	public void doRender(EntityMoiraOrb entity, double x, double y, double z, float entityYaw, float partialTicks) {
		...
		for (EntityLivingBase tether : entity.tethered) {
			...
			buffer.pos(tetherVec.x, tetherVec.y+tether.height*0.7f, tetherVec.z).tex(diff/2f+sprite.getMinU()+diff*0.2f, sprite.getMinV()+diff*0.2f).endVertex();
			...
			for (int j=0; j<2; j++) {
				...
				for (int i=1; i<=steps; i++) {
					...
					buffer.pos(x+target.x, y+target.y+entity.height*0.5f, z+target.z).tex(((i-1)/(steps-1))*diff*0.6f+sprite.getMinU()+diff*0.2f, sprite.getMaxV()-diff*0.2f).endVertex();
				}
