...
public class TileEntityHealthPackRenderer extends TileEntityOBJRenderer<TileEntityHealthPack> {
	...
	protected boolean preRender(TileEntityHealthPack te, int model, VertexBuffer buffer, double x, double y, double z, float partialTicks) {
		...
		if ((te.getBlockType() == ModBlocks.healthPackSmall && (model == 2 || model == 4)) || 
			...
			GlStateManager.translate(-0.01d, Math.sin((Minecraft.getMinecraft().thePlayer.ticksExisted+partialTicks)/5f)/10f+0.2d, -0.2d);
			GlStateManager.rotate(25, 1, 0, 0);
			GlStateManager.rotate((Minecraft.getMinecraft().thePlayer.ticksExisted+partialTicks)*4f, 0, 1, 0);
		}
	...
	protected void postRender(TileEntityHealthPack te, int model, VertexBuffer buffer, double x, double y, double z, float partialTicks) {
		...
		if (model == 0 && (te.getCooldown() > 0 || te.isHacked())) { 
			...
			if (te.isHacked() && te.hackedIconTime <= 0) {
				...
				// progress layer
				progress = MathHelper.clamp_double(0.92f -(float) te.hackedTime / TileEntityHealthPack.HACK_TIME, 0, 1);
				if (friendly)
