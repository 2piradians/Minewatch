...
public class CommonProxy {
	...
	public void createExplosion(World worldObj, Entity exploder, double x, double y, double z, float size, float exploderDamage, float minDamage, float maxDamage, @Nullable Entity directHit, float directHitDamage, boolean resetHurtResist, float exploderKnockback, float knockback) {
		if (!worldObj.isRemote) {
			Explosion explosion = new Explosion(worldObj, exploder, x, y, z, size, false, false);
			...
			List<Entity> list = worldObj.getEntitiesWithinAABBExcludingEntity(null, new AxisAlignedBB((double)k1, (double)i2, (double)j2, (double)l1, (double)i1, (double)j1));
			net.minecraftforge.event.ForgeEventFactory.onExplosionDetonate(worldObj, explosion, list, f3);
				...
				if (!entity.isImmuneToExplosions() && (!(entity instanceof EntityLivingBase) || 
					...
					if (d12 <= 1.0D) {
						...
						if (d13 != 0.0D) {
							...
							d7 = d7 / d13;
							d9 = d9 / d13; 
							double d14 = 1;//(double)worldObj.getBlockDensity(vec3d, entity.getEntityBoundingBox());
