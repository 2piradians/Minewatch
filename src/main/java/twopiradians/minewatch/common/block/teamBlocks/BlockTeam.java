...
public abstract class BlockTeam extends Block{
	...
	@Override
	public int getLightValue(IBlockState state, IBlockAccess worldObj, BlockPos pos) {
		return 8;
	}
	...
	public int colorMultiplier(IBlockState state, IBlockAccess worldObj, BlockPos pos, int tintIndex) {
		if (worldObj != null && pos != null && worldObj.getTileEntity(pos) != null && worldObj.getTileEntity(pos) instanceof TileEntityTeam) {
    ...
    public boolean onBlockActivated(World worldObj, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, @Nullable ItemStack heldItem, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (!player.worldObj.isRemote && !player.isSneaking() && worldObj.getTileEntity(pos) instanceof TileEntityTeam &&
