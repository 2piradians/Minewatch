...
public class CommandMinewatch implements ICommand {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		...
		// teamSpawn <name> <activate|deactivate>
			...
			if (pos == null || !(sender.getEntityWorld().getTileEntity(pos) instanceof TileEntityTeamSpawn))
				sender.addChatMessage(new TextComponentTranslation(TextFormatting.RED+args[1]+" is not a valid Team Spawn name or is not in this dimension"));
			else {
				...
				sender.addChatMessage(new TextComponentTranslation(TextFormatting.GREEN+args[1]+" is now: "+(activate ? TextFormatting.DARK_GREEN+"Activated" : TextFormatting.DARK_RED+"Deactivated")));
			}
		}
		// reset
			...
			if (!flagStrings.isEmpty())
				sender.addChatMessage(new TextComponentString(TextFormatting.RED+Minewatch.translate("command.reset.unrecognized_flags")+": "+flagStrings));

			// notify all players of reset
			for (EntityPlayer player : sender.getEntityWorld().playerEntities)
				player.addChatMessage(new TextComponentString(TextFormatting.YELLOW+Minewatch.translate("command.reset.success")));
		}
		else
	...
	public static void equipWithHeroArmor(EnumHero hero, EntityLivingBase entity, ICommandSender sender) {
		if (!(entity instanceof EntityLivingBaseMW)) {
			...
			sender.addChatMessage(new TextComponentTranslation(TextFormatting.GREEN+"Spawned set for "+hero.getFormattedName(false)+
