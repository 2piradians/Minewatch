		...
		if (this.isFriendly) {
			...
			if (this.worldObj.isRemote && result.entityHit != null) {
				...
				Minewatch.proxy.spawnParticlesCustom(EnumParticle.ANA_HEAL, worldObj, result.entityHit, 0xFFFFFF, 0xFFFFFF, 0.7f, 
						30+worldObj.rand.nextInt(10), size, size/1.5f, worldObj.rand.nextFloat(), (worldObj.rand.nextFloat()-0.5f)/5f);
				this.worldObj.playSound((EntityPlayer) this.getThrower(), this.getThrower().getPosition(), ModSoundEvents.anaHeal, SoundCategory.PLAYERS, 
						0.3f, result.entityHit.worldObj.rand.nextFloat()/2+1.5f);
				result.entityHit.playSound(ModSoundEvents.anaHeal, 0.2f, result.entityHit.worldObj.rand.nextFloat()/2+1.5f);
			}
		}
		// damage
