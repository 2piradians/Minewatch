...
public class EntityMercyBeam extends Entity {
	...
	public void onUpdate() {		
		...
		if (this.worldObj.isRemote && player == null && this.dataManager.get(PLAYER).isPresent())
			player = this.worldObj.getPlayerEntityByUUID(this.dataManager.get(PLAYER).get());
		// set target on client
		if (this.worldObj.isRemote && target == null && this.dataManager.get(TARGET) != 0) {
		...
		if (!this.worldObj.isRemote && (player == null || player.isDead || target == null || target.isDead || 
