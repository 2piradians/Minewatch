...
public class EntityWidowmakerMine extends EntityLivingBaseMW {
	...
	public void onUpdate() {
		...
		if (!this.world.isRemote && this.onGround && 
				this.facing != null && !world.collidesWithAnyBlock(getEntityBoundingBox().grow(0.01d))) {
			this.onGround = false;
			this.facing = null;
		...
		if (!this.world.isRemote  && this.onGround && this.getThrower() instanceof EntityLivingBase) {
			List<Entity> entities = this.world.getEntitiesWithinAABBExcludingEntity(this, this.getEntityBoundingBox().grow(3d));
