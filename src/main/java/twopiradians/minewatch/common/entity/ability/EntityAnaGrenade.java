...
public class EntityAnaGrenade extends EntityMW {
	...
	public void onImpact(RayTraceResult result) {
		...
		if (!worldObj.isRemote) {
			...
			boolean heal = false;
			for (EntityLivingBase entity : this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expandXyz(4))) {
				if (entity.isEntityAlive()) { 
					// heal
					if (EntityHelper.attemptDamage(getThrower(), entity, -100, true)) {
						TickHandler.register(false, HEAL.setEntity(entity).setTicks(80));
						Minewatch.network.sendToDimension(new SPacketSimple(53, entity, true), worldObj.provider.getDimension());
					...
					// damage
						...
						Minewatch.network.sendToDimension(new SPacketSimple(53, entity, false, entity.getHealth(), 0, 0), worldObj.provider.getDimension());
						damage = true;
					}
		...
		if (this.worldObj.isRemote) {
			...
			for (int i=0; i<4; ++i)
				Minewatch.proxy.spawnParticlesCustom(EnumParticle.ANA_DAMAGE, worldObj, posX, posY, posZ, 0, 0, 0, 0xFFFFFF, 0xFFFFFF, 1f, 9, 5, 40, worldObj.rand.nextFloat(), 0);
			Minewatch.proxy.spawnParticlesCustom(EnumParticle.CIRCLE, worldObj, posX, posY, posZ, 0, 0, 0, 0xB4A779, 0xB4A779, 0.8f, 12, 7, 37, 0, 0);
			Minewatch.proxy.spawnParticlesCustom(EnumParticle.HOLLOW_CIRCLE, worldObj, posX, posY, posZ, 0, 0, 0, 0x6646AB, 0x6646AB, 1f, 7, 5, 39, worldObj.rand.nextFloat(), 0.05f);
			Minewatch.proxy.spawnParticlesCustom(EnumParticle.CIRCLE, worldObj, posX, posY, posZ, 0, 0, 0, 0x6646AB, 0x6646AB, 1f, 4, 30, 35, 0, 0);
			Minewatch.proxy.spawnParticlesCustom(EnumParticle.CIRCLE, worldObj, posX, posY, posZ, 0, 0, 0, 0xFCF8E1, 0xFDF7DF, 1f, 4, 20, 25, 0, 0);
		}
