...
public class EntityJunkratMine extends EntityLivingBaseMW {
	...
	protected void applyEntityAttributes() {
		...
		this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(Math.max(1, 50D*Config.getDamageScale(null)));
	}
	...
	public void onUpdate() {	
		...
		if (!this.world.isRemote && this.onGround && 
				this.facing != null && !world.collidesWithAnyBlock(getEntityBoundingBox().grow(0.2d))) {
			this.onGround = false;
			this.facing = null;
	...
	public void explode() {
		if (this.world.isRemote) {
			...
			Minewatch.proxy.createExplosion(world, this, posX, posY, posZ, 3f, 0, 50, 120, null, 120, false, 2.2f, 2.2f);
