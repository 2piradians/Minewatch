	...
	public void spawnTrailParticles() {
		if (type == 0) {
			...
			if (type == 0 && this.ticksExisted % 2 == 0)
				Minewatch.proxy.spawnParticlesCustom(EnumParticle.CIRCLE, worldObj, posX, posY+height/2d, posZ, 0, 0, 0, 0x90E3FF, 0x91C3ED, 1f, 7, 2, 2.2f, 0, 0);
		}
		else if (type == 1)
				...
				Minewatch.proxy.spawnParticlesCustom(EnumParticle.SPARK, worldObj, result.entityHit == null ? result.hitVec.xCoord : posX, 
						result.entityHit == null ? result.hitVec.yCoord : posY, 
								result.entityHit == null ? result.hitVec.zCoord : posZ, 
										0, 0, 0, 0x86F3FF, 0xCFDFF9, 0.7f, 10, 5, 4.5f, worldObj.rand.nextFloat(), 0.01f);
		}
	}
}
