...
public enum EnumHero {
	...
	public void renderOverlay(EntityPlayer player, ScaledResolution resolution) {
		...
		if (ItemMWArmor.SetManager.playersWearingSets.containsKey(player.getPersistentID()) &&
			...
			if (player.getHeldItemMainhand() != null && player.getHeldItemMainhand().getItem() instanceof ItemMWWeapon) {
				...
				GlStateManager.translate((int) (resolution.getScaledWidth())-125, ((int)resolution.getScaledHeight()/4)-22, 0);
				...
				if (weapon.hero == this && ItemMWArmor.SetManager.playersWearingSets.containsKey(player.getPersistentID())) {
					// slot 1
					if (this.slot1.getCooldown(player) > 0) 
						GlStateManager.color(0.4f, 0.4f, 0.4f);
					else if (this.slot1.isKeyDown(player)) {
						GlStateManager.color(0.8f, 0.6f, 0);
						GlStateManager.translate(1, 1, 0);
					}
					GuiUtils.drawTexturedModalRect(-50, -2, 124, (index+1)+index*vertical, 40, vertical, 0);
					GlStateManager.color(1, 1, 1);
					if (this.slot1.getCooldown(player) <= 0 && this.slot1.isKeyDown(player)) 
						GlStateManager.translate(-1, -1, 0);
					// slot 2
					if (this.slot2.getCooldown(player) > 0) 
						GlStateManager.color(0.4f, 0.4f, 0.4f);
					else if (this.slot2.isKeyDown(player)) {
						GlStateManager.color(0.8f, 0.6f, 0);
						GlStateManager.translate(1, 1, 0);
					}
					GuiUtils.drawTexturedModalRect(-87, -2, 165, (index+1)+index*vertical, 40, vertical, 0);
					GlStateManager.color(1, 1, 1);
					if (this.slot2.getCooldown(player) <= 0 && this.slot2.isKeyDown(player)) 
						GlStateManager.translate(-1, -1, 0);
					// slot 3
					if (this.slot3.getCooldown(player) > 0) 
						GlStateManager.color(0.4f, 0.4f, 0.4f);
					else if (this.slot3.isKeyDown(player)) {
						GlStateManager.color(0.8f, 0.6f, 0);
						GlStateManager.translate(1, 1, 0);
					}
					GuiUtils.drawTexturedModalRect(-124, -2, 206, (index+1)+index*vertical, 40, vertical, 0);
					GlStateManager.color(1, 1, 1);
					if (this.slot3.getCooldown(player) <= 0 && this.slot3.isKeyDown(player)) 
						GlStateManager.translate(-1, -1, 0);

					// keybinds 
					int width1 = Minecraft.getMinecraft().fontRendererObj.getStringWidth(this.slot1.getKeyName());
					int width2 = Minecraft.getMinecraft().fontRendererObj.getStringWidth(this.slot2.getKeyName());
					int width3 = Minecraft.getMinecraft().fontRendererObj.getStringWidth(this.slot3.getKeyName());
					// background
					// slot 1
					if (this.slot1.getCooldown(player) > 0) 
						GlStateManager.color(0.4f, 0.4f, 0.4f);
					if (this.slot1.getKeyName() != "")
						GuiUtils.drawTexturedModalRect(-58, 3, 0, 1015, 40, 9, 0);
					else if (this.slot1 == KeyBind.RMB)
						GuiUtils.drawTexturedModalRect(-43, 3, 46, 1015, 10, 9, 0);
					GlStateManager.color(1, 1, 1);
					// slot 2
					if (this.slot2.getCooldown(player) > 0) 
						GlStateManager.color(0.4f, 0.4f, 0.4f);
					if (this.slot2.getKeyName() != "")
						GuiUtils.drawTexturedModalRect(-98, 2, 0, 1015, 40, 9, 0);
					GlStateManager.color(1, 1, 1);
					// slot 3
					if (this.slot3.getCooldown(player) > 0) 
						GlStateManager.color(0.4f, 0.4f, 0.4f);
					if (this.slot3.getKeyName() != "")
					...
					// slot 1
					Minecraft.getMinecraft().fontRendererObj.drawString(this.slot1.getKeyName(), -33-width1/2, 38, 0);
					// slot 2
					Minecraft.getMinecraft().fontRendererObj.drawString(this.slot2.getKeyName(), -74-width2/2, 37, 0);
					// slot 3
					Minecraft.getMinecraft().fontRendererObj.drawString(this.slot3.getKeyName(), -114-width3/2, 37, 0);

					// cooldowns
					double scale = 2d;
					GlStateManager.scale(scale, scale, 1);
					if (this.slot1.getCooldown(player) > 0) { 
						String num = String.valueOf(this.slot1.getCooldown(player)/20);
					...
					if (this.slot2.getCooldown(player) > 0) { 
						String num = String.valueOf(this.slot2.getCooldown(player)/20);
						int width = Minecraft.getMinecraft().fontRendererObj.getStringWidth(num);
						Minecraft.getMinecraft().fontRendererObj.drawString(num, -33-width/2, 4, 0xFFFFFF);
					}
					if (this.slot3.getCooldown(player) > 0) { 
						String num = String.valueOf(this.slot3.getCooldown(player)/20);
						int width = Minecraft.getMinecraft().fontRendererObj.getStringWidth(num);
				...
				if (weapon.getMaxAmmo(player) > 0) {
					if (weapon.hero != this || 
							!ItemMWArmor.SetManager.playersWearingSets.containsKey(player.getPersistentID())) { // adjust things that were skipped
						GlStateManager.scale(1, 0.25d, 1);
						GlStateManager.rotate(4.5f, 0, 0, 1);
						double scale = 2d;
						GlStateManager.scale(scale, scale, 1);
