...
public class RenderManager {
	...
	public static void hurtTime(TickEvent.RenderTickEvent event) {
		...
		if (Minecraft.getMinecraft().thePlayer != null && 
				SetManager.getWornSet(Minecraft.getMinecraft().thePlayer) != null)
	...
	public static void damageEntities(LivingHurtEvent event) {
		...
		if (player != null && event.getEntityLiving() != null && player != event.getEntityLiving()) {
			if (!player.worldObj.isRemote && SetManager.getWornSet(player) != null) {
				try {
			...
			for (UUID uuid : entityDamage.get(event.getEntityLiving()).keySet()) {
				...
				if (player instanceof EntityPlayerMP) {
					...
					if (percent >= 10 && entityDamage.get(event.getEntityLiving()).get(uuid).getSecond() > 0) {
						...
						if (SetManager.getWornSet(uuid, player.worldObj.isRemote) == EnumHero.GENJI) {
	...
	public static void renderOnBlocks(RenderWorldLastEvent event) {
		...
		for (Entity entity : Minecraft.getMinecraft().theWorld.loadedEntityList) {
			if (entity instanceof EntityLivingBase && SetManager.getWornSet(entity) == EnumHero.LUCIO && 
				...
				renderOnBlocks(entity.worldObj, buffer, 
	...
	public static void renderOnBlocks(World worldObj, VertexBuffer buffer, float red, float green, float blue, float alpha, double size, Vec3d entityVec, EnumFacing facing, boolean particle) {
		...
		double minX = MathHelper.floor_double(entityVec.xCoord - (facing.getAxis() == Axis.X ? 0 : size));
		double maxX = MathHelper.floor_double(entityVec.xCoord + (facing.getAxis() == Axis.X ? 0 : size));
		double minY = MathHelper.floor_double(entityVec.yCoord - (facing.getAxis() == Axis.Y ? 0 : size));
				...
				if (state.getRenderType() != EnumBlockRenderType.INVISIBLE && state.getRenderType() != EnumBlockRenderType.LIQUID) {
					...
					if (alpha >= 0.0D) {
						...
						case Y:
							...
							double f = MathHelper.clamp_double(((diffVec.xCoord - minX) / 2.0D / size + 0.5D), 0, 1);
							double f1 = MathHelper.clamp_double(((diffVec.xCoord - maxX) / 2.0D / size + 0.5D), 0, 1);
							double f2 = MathHelper.clamp_double(((diffVec.zCoord - minZ) / 2.0D / size + 0.5D), 0, 1);
							double f3 = MathHelper.clamp_double(((diffVec.zCoord - maxZ) / 2.0D / size + 0.5D), 0, 1);
						...
						case Z:
							...
							f = MathHelper.clamp_double(((diffVec.xCoord - minX) / 2.0D / size + 0.5D), 0, 1);
							f1 = MathHelper.clamp_double(((diffVec.xCoord - maxX) / 2.0D / size + 0.5D), 0, 1);
							f2 = MathHelper.clamp_double(((diffVec.yCoord - minY) / 2.0D / size + 0.5D), 0, 1);
							f3 = MathHelper.clamp_double(((diffVec.yCoord - maxY) / 2.0D / size + 0.5D), 0, 1);
						...
						case X:
							...
							f = MathHelper.clamp_double(((diffVec.zCoord - minZ) / 2.0D / size + 0.5D), 0, 1);
							f1 = MathHelper.clamp_double(((diffVec.zCoord - maxZ) / 2.0D / size + 0.5D), 0, 1);
							f2 = MathHelper.clamp_double(((diffVec.yCoord - minY) / 2.0D / size + 0.5D), 0, 1);
							f3 = MathHelper.clamp_double(((diffVec.yCoord - maxY) / 2.0D / size + 0.5D), 0, 1);
