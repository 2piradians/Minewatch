				...
				if (weapon != null) {
					...
					if (hero != null && weapon.hero == hero && SetManager.getWornSet(player) != null) {
						for (int i=1; i<=3; ++i) {
							...
							// text
							int width = Minecraft.getMinecraft().fontRenderer.getStringWidth(ability.keybind.getKeyName());
							GlStateManager.scale(0.25d, 0.25d, 1);
							GlStateManager.rotate(4.5f, 0, 0, 1);
							// keybind text
							if (ability.showKeybind(player)) 
								Minecraft.getMinecraft().fontRenderer.drawString(ability.keybind.getKeyName(), 3-i*36-width/2, 43+i*3, 0);
							// multi-use number
							if (ability.maxUses > 0)
								Minecraft.getMinecraft().fontRenderer.drawString(String.valueOf(ability.getUses(player)), 16-i*36, -8+i*3, 0);
							// cooldown
							scale = 2d;
							GlStateManager.scale(scale, scale, 1);
							if (ability.getCooldown(player) > 0) { 
								String num = String.valueOf((int)Math.ceil(ability.getCooldown(player)/20d));
								width = Minecraft.getMinecraft().fontRenderer.getStringWidth(num);
								Minecraft.getMinecraft().fontRenderer.drawString(num, 6-i*18-width/2, 8+i, 0xFFFFFF);
							}
							GlStateManager.color(1, 1, 1);
							GlStateManager.popMatrix();
						}
					...
					if (weapon.getMaxAmmo(player) > 0) {
						...
						scale = 0.6d;
						GlStateManager.scale(scale, scale, 1);
						Minecraft.getMinecraft().fontRenderer.drawString("/", 53, -10, 0x00D5FF);
						Minecraft.getMinecraft().fontRenderer.drawString(
