...
public class RespawnManager {
	...
	public static final Handler DEAD = new Handler(Identifier.DEAD, false) {
		...
		public boolean onClientTick() {
			...
			if ((Minecraft.getMinecraft().gameSettings.keyBindUseItem.isKeyDown() || 
				...
				if (string != null && !this.bool) {
					ArrayList<EntityLivingBase> teamEntities = new ArrayList<EntityLivingBase>(player.worldObj.getEntities(EntityLivingBase.class, new Predicate<EntityLivingBase>() {
						@Override
		...
		public Handler onServerRemove() {
			...
			respawnEntityInWorld(entityLiving, entityLiving.worldObj.getScoreboard().getTeam(string), false); 
			return super.onServerRemove();
		}
	};

	/**Respawn a player / mob*/
	public static void respawnEntityInWorld(EntityLivingBase entity, @Nullable Team team, boolean allowAlive) {
		...
		if (entity instanceof EntityPlayerMP) {
			...
			if (allowAlive && player.isEntityAlive()) {
				if (teamSpawn != null) {
					BlockPos fuzz = spawnFuzz(teamSpawn, player.worldObj);
			...
			// move spawn point, spawn, move back spawn point
				...
				if (teamSpawn != null) 
					player.setSpawnPoint(spawnFuzz(teamSpawn, player.worldObj), true);
			...
			if (teamSpawn != null) {
				try {
					EnumFacing facing = player.worldObj.getBlockState(teamSpawn).getValue(BlockTeamSpawn.FACING);
			...
			try {
				...
				EnumFacing facing = entity.worldObj.getBlockState(teamSpawn).getValue(BlockTeamSpawn.FACING);
				teamSpawn = spawnFuzz(teamSpawn, entity.worldObj);
	...
	public static BlockPos getTeamSpawn(EntityLivingBase entity, @Nullable Team team) {
		...
		for (BlockPos pos : TileEntityTeamSpawn.teamSpawnPositions.keySet())
			if (entity != null && pos != null && entity.worldObj.getTileEntity(pos) instanceof TileEntityTeamSpawn) {
				TileEntityTeamSpawn te = ((TileEntityTeamSpawn)entity.worldObj.getTileEntity(pos));
	...
	public static BlockPos spawnFuzz(BlockPos pos, World worldObj) {
		if (pos != null && worldObj.getTileEntity(pos) instanceof TileEntityTeamSpawn) {
			TileEntityTeamSpawn te = (TileEntityTeamSpawn) worldObj.getTileEntity(pos);
			...
			for (BlockPos newPos : positions) {
				...
				while (newPos.getDistance(te.getPos().getX(), te.getPos().getY(), te.getPos().getZ()) <= radius+1) {
					if (EntityHelper.isValidTeleportLocation(newPos, worldObj)) // valid spot found
						return newPos;
					newPos = newPos.up();
				}
			}
	...
	public static void respawnPlayers(PlayerRespawnEvent event) {
		...
		if (!event.player.worldObj.isRemote && !Config.customDeathScreen) {
			respawnEntityInWorld(event.player, event.player.getTeam(), true);
		}
	}
	...
	public static void registerMobs(LivingDeathEvent event) {
		...
		if (isRespawnableEntity(event.getEntityLiving()) &&
				!event.getEntityLiving().worldObj.isRemote && !TickHandler.hasHandler(event.getEntityLiving(), Identifier.DEAD)) {
