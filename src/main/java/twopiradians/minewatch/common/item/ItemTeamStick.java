...
public class ItemTeamStick extends Item {
	...
	public ItemTeamStick() {
		...
		this.addPropertyOverride(new ResourceLocation("hasTeam"), new IItemPropertyGetter() {
			...
			public float apply(ItemStack stack, @Nullable World worldObj, @Nullable EntityLivingBase entity) {
	...
	@Nullable
	public static ScorePlayerTeam getTeam(World worldObj, ItemStack stack) {
		String name = getTeamName(stack, false);
		if (name != null) 
			return worldObj.getScoreboard().getTeam(TextFormatting.getTextWithoutFormattingCodes(name));
		else
			return null;
	}
	...
	public boolean onLeftClickEntity(ItemStack stack, EntityPlayer player, Entity entity) {
		if (!player.worldObj.isRemote && entity instanceof EntityLivingBase && !EntityHelper.shouldIgnoreEntity(entity)) {
			Team team = getTeam(player.worldObj, stack);
			// copy team
			if (player.isSneaking()) {
			...
			// add to team
				...
				player.worldObj.getScoreboard().addPlayerToTeam(entity instanceof EntityPlayer ? entity.getName() : entity.getCachedUniqueIdString(), team.getRegisteredName());
				sendChatMessage(player, "Set "+entity.getName()+"'s team to: "+getTeamName(stack, true));
				player.worldObj.playSound(null, player.getPosition(), SoundEvents.BLOCK_LEVER_CLICK, SoundCategory.PLAYERS, 0.5f, 1.8f);
			}
	...
	public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer player, EntityLivingBase entity, EnumHand hand) {
		if (!player.worldObj.isRemote && !player.isSneaking() && !EntityHelper.shouldIgnoreEntity(entity)) {
			// remove from team
			if (entity.getTeam() != null) {
				try {
					player.worldObj.getScoreboard().removePlayerFromTeams(entity instanceof EntityPlayer ? entity.getName() : entity.getCachedUniqueIdString());
				}
				catch (Exception e) {}
				sendChatMessage(player, "Removed "+entity.getName()+"'s team");
				player.worldObj.playSound(null, player.getPosition(), SoundEvents.ENTITY_ITEMFRAME_REMOVE_ITEM, SoundCategory.PLAYERS, 0.8f, 1);
				return true;
			}
	...
	public void onUpdate(ItemStack stack, World worldObj, Entity entity, int itemSlot, boolean isSelected) {
		...
		if (!worldObj.isRemote && entity.ticksExisted % 20 == 0) 
			setTeam(stack, getTeam(worldObj, stack));
	}

	@Override
	public boolean onBlockStartBreak(ItemStack stack, BlockPos pos, EntityPlayer player) {
		if (!player.worldObj.isRemote && player.worldObj.getTileEntity(pos) instanceof TileEntityTeam) {
			TileEntityTeam te = (TileEntityTeam) player.worldObj.getTileEntity(pos);
	...
	public static int getColorFromItemStack(ItemStack stack, int tintIndex) {
		if (tintIndex == 1 || Minecraft.getMinecraft().thePlayer == null)
			return -1;

		TextFormatting format = getTeamFormat(stack);
		int rate = 20;
		float glow = (Minecraft.getMinecraft().thePlayer.ticksExisted % rate);
		if (glow > rate/2)
			glow = rate-glow;
		glow *= 2f;
		if (format.isColor()) {
			Color color = new Color(ColorHelper.getForegroundColor(format));
			return new Color((int) MathHelper.clamp_double(color.getRed()+glow, 0, 255), 
					(int) MathHelper.clamp_double(color.getGreen()+glow, 0, 255), 
					(int) MathHelper.clamp_double(color.getBlue()+glow, 0, 255)).getRGB();
		}
		return -1;
	}
