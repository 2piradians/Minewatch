...
public class ModArmor extends ItemArmor 
{
	...
	public ModArmor(Hero hero, ArmorMaterial materialIn, int renderIndexIn, EntityEquipmentSlot equipmentSlotIn) {
	...
	public static class SetManager {
		...
		public static void updateSets(TickEvent.PlayerTickEvent event) {			
			if (event.phase == TickEvent.Phase.START) {
				...
				Hero hero = null;
				boolean fullSet = helm != null && helm.getItem() instanceof ModArmor;
				if (fullSet) {
					hero = ((ModArmor)helm.getItem()).hero;
					for (EntityEquipmentSlot slot : SLOTS) {
						ItemStack armor = event.player.getItemStackFromSlot(slot);
						if (armor == null || !(armor.getItem() instanceof ModArmor)
								|| ((ModArmor)(armor.getItem())).hero != hero) 
							fullSet = false;
					}
				}
