...
public class ItemDoomfistWeapon extends ItemMWWeapon {
	...
	public static final Handler RELOAD = new Handler(Identifier.DOOMFIST_RELOAD, false) {
		...
		public Handler onServerRemove() {
			if (entityLiving != null && entityLiving.getHeldItemMainhand() != null && 
				...
				ModSoundEvents.DOOMFIST_RELOAD.playFollowingSound(entityLiving, 0.7f, entityLiving.worldObj.rand.nextFloat()/2+0.75f, false);
			}
	...
	public static final Handler SLAM = new Handler(Identifier.DOOMFIST_SLAM, true) {
		public void move() {
			...
			if (this.ticksLeft == this.initialTicks) { // moving up for first tick
				//if (entity.worldObj.isRemote) // debug particle
				//	Minewatch.proxy.spawnParticlesCustom(EnumParticle.CIRCLE, entity.worldObj, position.xCoord, position.yCoord+0.05d, position.zCoord, 0, 0, 0, 0xFF0000, 0xFF0000, 1, 300, 10, 10, 0, 0, EnumFacing.UP, false);
				...
				if (this.ticksLeft == this.initialTicks-1) {
					number2 = entity.posY;
					TickHandler.register(entity.worldObj.isRemote,	Handlers.PREVENT_MOVEMENT.setEntity(entity).setTicks(50).setBoolean(true));
				}
				...
				double verticalPercent = MathHelper.clamp_double((verticalDistance / Math.max(0.1d, Math.abs(number2-position.yCoord))), 0, 1d); 
				double scale = MathHelper.clamp_double(horizontalDistance, 0, 2.5d); // scale speed by horizontal distance
				...
				this.ticksLeft = 1;
				if (entity.onGround) {
					if (!entity.worldObj.isRemote) { // particles
						...
						for (int i=0; i<5; ++i) {
							...
							if (state.getRenderType() != EnumBlockRenderType.INVISIBLE && state.getRenderType() != EnumBlockRenderType.LIQUID) {
								yOffset += state.getBoundingBox(entity.worldObj, pos).maxY;
								...
								Minewatch.network.sendToDimension(new SPacketSimple(67, entity, false, vec.xCoord, vec.yCoord, vec.zCoord), entity.worldObj.provider.getDimension());
								break;
							}
							else
								pos = pos.down();
						...
						yOffset = 0;
						for (int i=0; i<3; ++i) {
							IBlockState state = entity.worldObj.getBlockState(new BlockPos(entity.getPositionVector().addVector(0, -i, 0)));
							if (state.getRenderType() != EnumBlockRenderType.INVISIBLE && state.getRenderType() != EnumBlockRenderType.LIQUID) {
								yOffset = -i;
								break;
							}
						}
						int damage = MathHelper.clamp_int(getSlamCharge((EntityLivingBase) entity), 11, 125);
	...
	private static void moveUppercut(Handler handler) {
		...
		if (++handler.number == 4 && !handler.entity.worldObj.isRemote &&
		...
		if (!handler.entity.worldObj.isRemote && handler.identifier == Identifier.DOOMFIST_UPPERCUTTING &&
				handler.ticksLeft > 2) {
			Vec3d look = handler.entity.getLookVec().scale(1d);
			AxisAlignedBB aabb = handler.entity.getEntityBoundingBox().expandXyz(1d).addCoord(look.xCoord, look.yCoord, look.zCoord);
			for (Entity target : handler.entity.worldObj.getEntitiesWithinAABBExcludingEntity(handler.entity, aabb)) 
				...
				EntityHelper.shouldHit(handler.entity, target, false)) {
					if (target.isEntityAlive() && !TickHandler.hasHandler(target, Identifier.DOOMFIST_UPPERCUT) && 
						...
						Minewatch.network.sendToDimension(new SPacketSimple(63, handler.entity, true, target, handler.ticksLeft+1, 0, 0), handler.entity.worldObj.provider.getDimension());
		...
		public Handler onClientRemove() {
			if (entity instanceof EntityLivingBase) {
				...
				if (result != null) {
					...
					Minewatch.proxy.spawnParticlesCustom(EnumParticle.DOOMFIST_PUNCH_0, entity.worldObj, pos.xCoord, pos.yCoord, pos.zCoord, 0, 0, 0, 0xFFFFFF, 0xFFFFFF, 1.0f, 300, 15, 15, entity.worldObj.rand.nextFloat(), 0, result.sideHit, true);
					Minewatch.proxy.spawnParticlesCustom(EnumParticle.DOOMFIST_PUNCH_1, entity.worldObj, pos.xCoord, pos.yCoord, pos.zCoord, 0, 0, 0, 0xFFFFFF, 0x62B4FE, 0.7f, 10, 15, 15, entity.worldObj.rand.nextFloat(), 0, result.sideHit, true);
				}
			}
			if (entity instanceof EntityLivingBase) {
				entity.motionX = 0;
	...
	private static void movePunch(Handler handler) {
			...
			for (Entity target : handler.entity.worldObj.getEntitiesWithinAABBExcludingEntity(handler.entity, aabb)) 
				...
				((EntityLivingBase)handler.entity).canEntityBeSeen(target)) {
					/*if (target.isEntityAlive() && (TickHandler.hasHandler(target, Identifier.REINHARDT_CHARGE) ||
						...
						Minewatch.network.sendToDimension(new SPacketSimple(58, handler.entity, true, target), handler.entity.worldObj.provider.getDimension());
					}
						...
						TickHandler.interrupt(target);
						Minewatch.network.sendToDimension(new SPacketSimple(62, handler.entity, false, target, handler.initialTicks*0.7f, handler.number, handler.entity.rotationYaw), handler.entity.worldObj.provider.getDimension());
		...
		AxisAlignedBB aabb = handler.entity.getEntityBoundingBox().expand(0, -0.1d, 0).
				addCoord(EntityHelper.getLook(0, (float) (handler.identifier == Identifier.DOOMFIST_PUNCH ? handler.entity.getRotationYawHead() : handler.number2)).scale(1).xCoord,
						EntityHelper.getLook(0, (float) (handler.identifier == Identifier.DOOMFIST_PUNCH ? handler.entity.getRotationYawHead() : handler.number2)).scale(1).yCoord,
						EntityHelper.getLook(0, (float) (handler.identifier == Identifier.DOOMFIST_PUNCH ? handler.entity.getRotationYawHead() : handler.number2)).scale(1).zCoord);
		handler.entity.rotationPitch = pitch;
		if (handler.entity.worldObj.collidesWithAnyBlock(aabb)) {
			handler.ticksLeft = 1;
			handler.entity.motionX = 0;
			handler.entity.motionZ = 0;
			handler.entity.velocityChanged = true;
			if (handler.entity.worldObj.isRemote && handler.identifier == Identifier.DOOMFIST_PUNCH) {
				handler.bool = true;
			...
			else if (!handler.entity.worldObj.isRemote && handler.identifier == Identifier.DOOMFIST_PUNCHED && 
	...
	@Override
	public ItemStack onItemUseFinish(ItemStack stack, World worldObj, EntityLivingBase player) {
	...
	public void onItemLeftClick(ItemStack stack, World worldObj, EntityLivingBase player, EnumHand hand) { 
		...
		// shoot
		else if (this.canUse(player, true, hand, false) && !worldObj.isRemote) {
			for (int i=0; i<11; ++i) {
				EntityDoomfistBullet projectile = new EntityDoomfistBullet(worldObj, player, EnumHand.OFF_HAND.ordinal());
				EntityHelper.setAim(projectile, player, player.rotationPitch, player.rotationYawHead, 80, 4F, EnumHand.OFF_HAND, 18, 0.6f);
				worldObj.spawnEntityInWorld(projectile);
			}
			ModSoundEvents.DOOMFIST_SHOOT.playSound(player, worldObj.rand.nextFloat()+0.5F, worldObj.rand.nextFloat()/2+0.75f);
	...
	@Override
	public void onPlayerStoppedUsing(ItemStack stack, World worldObj, EntityLivingBase player, int timeLeft) {
		// release punch
		float charge = getCharge(player);
		if (!worldObj.isRemote && charge > 0) {
			ModSoundEvents.DOOMFIST_PUNCH_CHARGE.stopFollowingSound(player);
			ModSoundEvents.DOOMFIST_PUNCH_CHARGE_VOICE.stopFollowingSound(player);
			...
			int ticks = 8;
			Minewatch.network.sendToDimension(new SPacketSimple(62, player, true, ticks, 0, 0), worldObj.provider.getDimension());
	...
	public ActionResult<ItemStack> onItemRightClick(World worldObj, EntityLivingBase player, EnumHand hand) {
		...
		// charge punch
		if (!worldObj.isRemote && hand == EnumHand.MAIN_HAND && this.canUse(player, true, hand, true) && 
			...
			Minewatch.network.sendToDimension(new SPacketSimple(66, player, false), worldObj.provider.getDimension());
	...
	public void onUpdate(ItemStack stack, World worldObj, Entity entity, int slot, boolean isSelected) {	
		...
		if (isSelected && entity instanceof EntityLivingBase && ((EntityLivingBase) entity).getHeldItemMainhand() == stack) {	
			...
			// slam particle
			if (player == Minewatch.proxy.getClientPlayer() && worldObj.isRemote && hero.ability2.keybind.getCooldown(player) <= 0 && 
					player.ticksExisted % 5 == 0 && !player.onGround)
				Minewatch.proxy.spawnParticlesCustom(EnumParticle.DOOMFIST_SLAM_0, worldObj, player, 0xFFFFFF, 0xFFFFFF, 1, Integer.MAX_VALUE, 60, 60, 0, 0);

			// reload automatically
			if (!worldObj.isRemote && this.getCurrentAmmo(player) < this.getMaxAmmo(player) && !TickHandler.hasHandler(player, Identifier.DOOMFIST_RELOAD)) 
				TickHandler.register(false, RELOAD.setEntity(player).setTicks(this.reloadTime+1));

			// seismic slam
			if (hero.ability2.isSelected(player) &&
					!worldObj.isRemote && this.canUse(player, true, getHand(player, stack), true)) {
				...
				if ((result != null && result.hitVec.yCoord <= player.posY) || player.onGround) {
					...
					Minewatch.network.sendToDimension(new SPacketSimple(64, player, true, targetPos.xCoord, targetPos.yCoord, targetPos.zCoord), worldObj.provider.getDimension());
			...
			if (hero.ability3.isSelected(player) &&
				...
				int ticks = 4;
				Minewatch.network.sendToDimension(new SPacketSimple(63, player, true, ticks, 0, 0), worldObj.provider.getDimension());
	...
	public static float getCharge(EntityLivingBase entity) {
		if (entity != null && entity.getActiveItemStack() != null && entity.getActiveItemStack().getItem() == EnumHero.DOOMFIST.weapon) {
			float charge = MathHelper.clamp_float(2f - (float) entity.getItemInUseCount() / 20f, 0, 1);
			return charge;
		}
		return -1;
	}
	...
	public int getColorFromItemStack(ItemStack stack, int tintIndex) {
		EntityLivingBase entity = getEntity(Minecraft.getMinecraft().theWorld, stack);
		int model = getModel(entity);
		if (model > 0 && entity.getHeldItemMainhand() == stack) {
			float percent = MathHelper.clamp_float(model == 1 ? ItemDoomfistWeapon.getCharge(entity) : 1, 0, 1);
	...
	public boolean renderOffHand(EntityLivingBase entity) {
		return entity != null && entity.getHeldItemOffhand() == null &&
