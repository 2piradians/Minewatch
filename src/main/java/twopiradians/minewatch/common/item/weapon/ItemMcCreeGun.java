	...
	public void onItemLeftClick(ItemStack stack, World world, EntityPlayer player, EnumHand hand) { 
		...
		if (this.canUse(player, true, hand)) {
			if (!world.isRemote) {
				...
				Minewatch.proxy.spawnParticlesSpark(world, vec.x, vec.y, vec.z, 0xFFEF89, 0x5A575A, 5, 1);
			}
		}
	}
	...
	public void onUsingTick(ItemStack stack, EntityLivingBase entity, int count) {
		if (entity instanceof EntityPlayer && count % 2 == 0 && this.canUse((EntityPlayer) entity, true, getHand(entity, stack))) {
			...
			if (!entity.world.isRemote && hand != null) {
				...
				Minewatch.proxy.spawnParticlesSpark(entity.world, vec.x, vec.y, vec.z, 0xFFEF89, 0x5A575A, 5, 1);
			}
		}
	}
