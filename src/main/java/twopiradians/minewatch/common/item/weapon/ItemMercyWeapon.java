...
public class ItemMercyWeapon extends ItemMWWeapon {
	...
	public void addToNotRegening(LivingHurtEvent event) {
		...
		if (target instanceof EntityPlayer && !target.worldObj.isRemote &&
		...
		for (EntityMercyBeam beam : beams.values()) {
			if (beam.target == source && beam.player instanceof EntityPlayerMP && !beam.player.worldObj.isRemote &&
				...
				((EntityPlayerMP)beam.player).connection.sendPacket((new SPacketSoundEffect
						(ModSoundEvents.hurt, SoundCategory.PLAYERS, source.posX, source.posY, source.posZ, 
								0.3f, source.worldObj.rand.nextFloat()/2+0.75f)));
				break;
			}
		}
	}
	...
	public EntityLivingBase getMouseOver(EntityPlayer player) {
		...
		if (player != null) {
			double d0 = 14;
			Vec3d vec3d = ItemMWWeapon.getPositionEyes(player, 1);

			double d1 = d0;

			Vec3d vec3d1 = player.getLook(1.0F);
			Vec3d vec3d2 = vec3d.addVector(vec3d1.xCoord * d0, vec3d1.yCoord * d0, vec3d1.zCoord * d0);
			List<Entity> list = player.worldObj.getEntitiesInAABBexcluding(player, player.getEntityBoundingBox().addCoord(vec3d1.xCoord * d0, vec3d1.yCoord * d0, vec3d1.zCoord * d0).expand(1.0D, 1.0D, 1.0D), Predicates.and(EntitySelectors.NOT_SPECTATING, new Predicate<Entity>() {
