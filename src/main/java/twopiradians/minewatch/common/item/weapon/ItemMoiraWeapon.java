...
public class ItemMoiraWeapon extends ItemMWWeapon {
	...
	public static Handler ORB_SELECT = new Handler(Identifier.MOIRA_ORB_SELECT, true) {
		...
		public boolean onClientTick() {
			// stop handler
			if (entity == Minecraft.getMinecraft().thePlayer && entityLiving.getHeldItemMainhand() == null || 
		...
		public boolean onServerTick() {
			...
			// spawn orb
				...
				EntityMoiraOrb orb = new EntityMoiraOrb(entityLiving.worldObj, entityLiving, hand.ordinal(), heal);
				EntityHelper.setAim(orb, entityLiving, entityLiving.rotationPitch, entityLiving.rotationYawHead, 20, 0, hand, 30, 0);
				entityLiving.worldObj.spawnEntityInWorld(orb);
		...
		public Handler onServerRemove() {
			Minewatch.network.sendToDimension(new SPacketSimple(49, entityLiving, false), entityLiving.worldObj.provider.getDimension());
	...
	public static Handler DAMAGE = new Handler(Identifier.MOIRA_DAMAGE, true) {
		...
		public boolean onClientTick() {
			...
			// find new target / clear target
				...
				if (entityLiving == null || !entityLiving.isEntityAlive()) 
					...
					entityLiving = null;
				Minewatch.proxy.spawnParticlesMuzzle(EnumParticle.MOIRA_DAMAGE, entity.worldObj, (EntityLivingBase) entity, 0xFFFFFF, 0xFFFFFF, 1, 8, 1.1f, 1.1f, entity.worldObj.rand.nextFloat(), 0.1f, EnumHand.MAIN_HAND, 20, 0.6f);
				if (entityLiving != null)
					Minewatch.proxy.spawnParticlesCustom(EnumParticle.MOIRA_DAMAGE, entity.worldObj, entityLiving, 0xFFFFFF, 0xFFFFFF, 1, 8, 3, 3, entity.worldObj.rand.nextFloat(), 0.1f);
			}
			else 
				this.ticksLeft = 10;
			// sounds
		...
		@Override
		public Handler onServerRemove() {
			Minewatch.network.sendToDimension(new SPacketSimple(48, entity, false),  entity.worldObj.provider.getDimension());
			return super.onServerRemove();
		}
		@Override
		@SideOnly(Side.CLIENT)
		public Handler onClientRemove() {
			ModSoundEvents.MOIRA_DAMAGE_DURING_MISS.stopSound(Minecraft.getMinecraft().thePlayer);
			ModSoundEvents.MOIRA_DAMAGE_DURING_HIT.stopSound(Minecraft.getMinecraft().thePlayer);
			return super.onClientRemove();
		}
	};
	...
	public static Handler FADE = new Handler(Identifier.MOIRA_FADE, false) {
		...
		@SideOnly(Side.CLIENT)
		public Handler onClientRemove() {
			Minewatch.proxy.spawnParticlesCustom(EnumParticle.REAPER_TELEPORT_BASE_0, entityLiving.worldObj, 
					entityLiving.posX, entityLiving.posY+entityLiving.height/2f, entityLiving.posZ, 0, 0, 0, 0xFFFFFF, 0xAAAAAA, 1, 5, 5, 17, entityLiving.worldObj.rand.nextFloat(), 0.3f);
			return super.onClientRemove();
		}
		@Override
		public Handler onServerRemove() {
	...
	public void onItemLeftClick(ItemStack stack, World worldObj, EntityLivingBase player, EnumHand hand) { 	
		if (hand == EnumHand.OFF_HAND && this.canUse(player, true, hand, false) && this.getCurrentCharge(player) >= 1 && 
			...
			if (!worldObj.isRemote) {
				EntityMoiraHealEnergy energy = new EntityMoiraHealEnergy(worldObj, player, hand.ordinal());
				EntityHelper.setAim(energy, player, player.rotationPitch, player.rotationYawHead, 30, 0,  
						hand, 20, 0.6f);
				worldObj.spawnEntityInWorld(energy);
				if (KeyBind.LMB.isKeyPressed(player)) 
					ModSoundEvents.MOIRA_HEAL_START.playFollowingSound(player, worldObj.rand.nextFloat()+0.5F, 1, false);
				else if (player.ticksExisted % 24 == 0)
					ModSoundEvents.MOIRA_HEAL_DURING.playFollowingSound(player, worldObj.rand.nextFloat()+0.5F, 1, false);
	...
	public void onUpdate(ItemStack stack, World worldObj, Entity entity, int slot, boolean isSelected) {
		...
		if (isSelected && entity instanceof EntityLivingBase && ((EntityLivingBase) entity).getHeldItemMainhand() == stack &&
			...
			// fade
			if (hero.ability3.isSelected(player, true) && !worldObj.isRemote &&
				...
				Minewatch.network.sendToDimension(new SPacketSimple(47, player, false), worldObj.provider.getDimension());
				...
				Minewatch.proxy.spawnParticlesCustom(EnumParticle.REAPER_TELEPORT_BASE_0, worldObj, 
						player.posX, player.posY+player.height/2f, player.posZ, 0, 0, 0, 0xFFFFFF, 0xAAAAAA, 1, 5, 17, 5, worldObj.rand.nextFloat(), 0.3f);
			}

			// orb select
			if (hero.ability2.isSelected(player, player instanceof EntityPlayer) && !worldObj.isRemote && 
					this.canUse((EntityLivingBase) entity, true, EnumHand.MAIN_HAND, true)) {
				...
				if (handler == null) {
					...
					Minewatch.network.sendToDimension(new SPacketSimple(49, player, true), worldObj.provider.getDimension());
	...
	public ActionResult<ItemStack> onItemRightClick(World worldObj, EntityLivingBase player, EnumHand hand) {
		...
		if (!worldObj.isRemote && hand == EnumHand.MAIN_HAND && this.canUse(player, true, hand, false) &&
			...
			if (!TickHandler.hasHandler(player, Identifier.MOIRA_DAMAGE)) {
				...
				Minewatch.network.sendToDimension(new SPacketSimple(48, player, true, target), worldObj.provider.getDimension());
				ModSoundEvents.MOIRA_DAMAGE_START.playFollowingSound(player, worldObj.rand.nextFloat()+0.5F, worldObj.rand.nextFloat()/2+0.75f, false);
				if (target != null)
	...
	public void renderArms(RenderSpecificHandEvent event) {
		...
		if (event.getItemStack() != null && event.getItemStack().getItem() == this && 
				!TickHandler.hasHandler(Minecraft.getMinecraft().thePlayer, Identifier.MOIRA_FADE) && 
				((event.getHand() == EnumHand.MAIN_HAND && KeyBind.RMB.isKeyDown(Minecraft.getMinecraft().thePlayer) || 
				(event.getHand() == EnumHand.OFF_HAND && KeyBind.LMB.isKeyDown(Minecraft.getMinecraft().thePlayer) && 
				!KeyBind.RMB.isKeyDown(Minecraft.getMinecraft().thePlayer)) || 
				(!KeyBind.RMB.isKeyDown(Minecraft.getMinecraft().thePlayer) && !KeyBind.LMB.isKeyDown(Minecraft.getMinecraft().thePlayer))))) {
			GlStateManager.pushMatrix();
			...
			float f1 = MathHelper.sqrt_double(f7);
