...
public class ItemReaperShotgun extends ItemMWWeapon {
			...
			if (this.ticksLeft > 8) {
				...
				for (int i=0; i<3; ++i) {
					player.worldObj.spawnParticle(EnumParticleTypes.SMOKE_LARGE, 
							player.posX+player.worldObj.rand.nextDouble()-0.5d, 
							player.posY+player.worldObj.rand.nextDouble()*(firstPerson ? 0d : 1.5d), 
							player.posZ+player.worldObj.rand.nextDouble()-0.5d, 
	...
	public void damageEntities(LivingHurtEvent event) {
		if (event.getSource().getSourceOfDamage() instanceof EntityPlayer && event.getEntityLiving() != null) {
			...
			if (!player.worldObj.isRemote && TickHandler.hasHandler(event.getEntityLiving(), Identifier.REAPER_WRAITH))
				event.setCanceled(true);
		}
	}
