...
public class ItemReaperShotgun extends ItemMWWeapon {
	...
	public void onItemLeftClick(ItemStack stack, World world, EntityPlayer player, EnumHand hand) { 
		if (this.canUse(player, true) && !world.isRemote && !serverTps.containsKey(player) &&
			...
			Minewatch.network.sendToAllAround(new SPacketSpawnParticle(0, vec.x, vec.y, vec.z, 0xD93B1A, 0x510D30, 5, 5), 
			...
			if (isSelected && hero.ability1.isSelected(player) && this.canUse(player, true)) {   
				...
				if (Minewatch.keys.lmb(player)) {
					...
					if (tpVec != null && !world.isRemote && player instanceof EntityPlayerMP) {
						Minewatch.network.sendToAll(new SPacketTriggerAbility(1, player, Math.floor(tpVec.x)+0.5d, tpVec.y, Math.floor(tpVec.z)+0.5d));
						serverTps.put(player, new Tuple(70, new Vec3d(Math.floor(tpVec.x)+0.5d, tpVec.y, Math.floor(tpVec.z)+0.5d)));
			...
			for (EntityPlayer player : serverTps.keySet()) {
				if (serverTps.get(player).getFirst() > 1) {
					...
					if (serverTps.get(player).getFirst() == 40) {
						...
						player.setPositionAndUpdate(serverTps.get(player).getSecond().x, 
								serverTps.get(player).getSecond().y, 
								serverTps.get(player).getSecond().z);
