	...
	private Vec3d getTeleportPos(EntityLivingBase player) {
		try {
			...
			if (result != null && result.typeOfHit == RayTraceResult.Type.BLOCK && result.hitVec != null) {
				...
				if ((player.worldObj.isAirBlock(pos.up()) || state1.getCollisionBoundingBox(player.worldObj, pos.up()) == null ||
						state1.getCollisionBoundingBox(player.worldObj, pos.up()) == Block.NULL_AABB) && 
						(player.worldObj.isAirBlock(pos.up(2)) || state2.getCollisionBoundingBox(player.worldObj, pos.up(2)) == null ||
						state2.getCollisionBoundingBox(player.worldObj, pos.up(2)) == Block.NULL_AABB) && 
						!player.worldObj.isAirBlock(pos) && 
						state.getCollisionBoundingBox(player.worldObj, pos) != null &&
						state.getCollisionBoundingBox(player.worldObj, pos) != Block.NULL_AABB &&
						Math.sqrt(result.getBlockPos().distanceSq(player.posX, player.posY, player.posZ)) <= 35 &&
						(EntityHelper.canBeSeen(player.worldObj, player.getPositionVector().addVector(0, player.getEyeHeight(), 0), new Vec3d(pos.up(2).getX()+0.5d, pos.up(2).getY()+0.5d, pos.up(2).getZ()+0.5d)) ||
						EntityHelper.canBeSeen(player.worldObj, player.getPositionVector().addVector(0, player.getEyeHeight(), 0), new Vec3d(pos.up().getX()+0.5d, pos.up().getY()+0.5d, pos.up().getZ()+0.5d))))
					return new Vec3d(result.hitVec.xCoord, 
							result.getBlockPos().getY()+state.getCollisionBoundingBox(player.worldObj, pos).maxY+
