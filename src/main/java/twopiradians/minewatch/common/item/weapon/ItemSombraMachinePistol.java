...
public class ItemSombraMachinePistol extends ItemMWWeapon {
	...
	public static final Handler HACKED = new Handler(Identifier.SOMBRA_HACKED, false) {
		...
		public boolean onClientTick() {
			...
			if (this.ticksLeft == this.initialTicks)
				Minewatch.proxy.spawnParticlesCustom(EnumParticle.SOMBRA_HACK, entity.worldObj, entity, 
						0x8F40F7, 0x8F40F7, 0.8f, 20, 8*scale, 12*scale, 0, 0);
			if (this.ticksLeft % 5 == 0)
				Minewatch.proxy.spawnParticlesCustom(EnumParticle.SOMBRA_HACK_NUMBERS, entity.worldObj, 
						entity.posX+(entity.worldObj.rand.nextFloat()-0.5f)*scale, 
						entity.posY+entity.height/2f+(entity.worldObj.rand.nextFloat()-0.5f)*scale, 
						entity.posZ+(entity.worldObj.rand.nextFloat()-0.5f)*scale, 
						0, -0.02f, 0, 
						0x8F40F7, 0x8F40F7, 1f, 20, 8*scale, 7*scale, 0, 0);
			if (this.ticksLeft % 10 == 0 && this.ticksLeft > 20)
				Minewatch.proxy.spawnParticlesCustom(EnumParticle.SOMBRA_HACK_MESH, entity.worldObj, 
						entity.posX+(entity.worldObj.rand.nextFloat()-0.5f)*scale, 
						entity.posY+entity.height/2f+(entity.worldObj.rand.nextFloat()-0.5f)*scale, 
						entity.posZ+(entity.worldObj.rand.nextFloat()-0.5f)*scale, 
						(entity.worldObj.rand.nextFloat()-0.5f)*0.05f, 
						(entity.worldObj.rand.nextFloat()-0.5f)*0.05f, 
						(entity.worldObj.rand.nextFloat()-0.5f)*0.05f, 
						0x8F40F7, 0x8F40F7, 1, 100, 12*scale, 8*scale, 1f+(entity.worldObj.rand.nextFloat()-0.5f)*0.1f, (entity.worldObj.rand.nextFloat()-0.5f)*0.03f);
	...
	public static Handler HACK = new Handler(Identifier.SOMBRA_HACK, true) {
		...
		public boolean onClientTick() {
			// basic checks
			if ((entity == Minecraft.getMinecraft().thePlayer /*|| entity instanceof EntityHero*/) && 
			...
			// find new target / clear target
				...
				if ((entityLiving == null || !entityLiving.isEntityAlive()) && 
						(this.position == null || !(entity.worldObj.getTileEntity(new BlockPos(this.position)) instanceof TileEntityHealthPack))) { 
				...
				// check health pack
					...
					if (result == null || !(entity.worldObj.getTileEntity(result.getBlockPos()) instanceof TileEntityHealthPack) ||
							!checkTargetInShootingView((EntityLivingBase) entity, position.addVector(0.5d, 0.5d, 0.5d)))
						position = null;
				}
			}
			...
			for (int i=0; i<targetVecs.size(); ++i) {
				...
				if (entityLiving != null)
					Minewatch.proxy.spawnParticlesCustom(EnumParticle.CIRCLE, entity.worldObj, target.xCoord, target.yCoord, target.zCoord, 0, 0, 0, 0xB36BF7, 0xB36BF7, 0.7f, 1, 1, 5, 0, 0);

				if ((number-1) % 4 == 0 && i == targetVecs.size()-1) {
					Vector2f offsets = getOffsets(i);
					Minewatch.proxy.spawnParticlesMuzzle(EnumParticle.CIRCLE, entity.worldObj, (EntityLivingBase) entity, 0xF685F8, 0xC985F6, 1f, 5, 1.2f, 0.5f, 0, 0, EnumHand.OFF_HAND, offsets.x, offsets.y);
					Minewatch.proxy.spawnParticlesMuzzle(EnumParticle.CIRCLE, entity.worldObj, (EntityLivingBase) entity, 0xFFFFFF, 0xFFFFFF, 0.6f, 5, 0.6f, 0.2f, 0, 0, EnumHand.OFF_HAND, offsets.x, offsets.y);
				}
			}
		...
		public boolean onServerTick() {
			...
			if ((entityLiving != null || position != null)) {
				...
				if (++number > 16) {
					if (position != null) {
						TileEntity te = entity.worldObj.getTileEntity(new BlockPos(position));
					...
					//Minewatch.network.sendToDimension(new SPacketSimple(61, entity, false, entityLiving),  entity.worldObj.provider.getDimension());
					if (entityLiving != null) {
						...
						ModSoundEvents.SOMBRA_HACK_COMPLETE.playSound(entity.worldObj, position.xCoord, position.yCoord, position.zCoord, 1, 1);
					return true;
				}
			}
	...
	public void onUpdate(ItemStack stack, World worldObj, Entity entity, int slot, boolean isSelected) {	
		...
		if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).getHeldItemMainhand() == stack) {	
			...
			// invisibility
			if (handler == null && !worldObj.isRemote && hero.ability3.isSelected(player, false, hero.ability2) && hero.ability3.keybind.isKeyDown(player) && 
			...
			if (!worldObj.isRemote && player.getHeldItemOffhand() == null && 
				...
				if (!TickHandler.hasHandler(player, Identifier.SOMBRA_HACK)) {
					...
					Minewatch.network.sendToDimension(new SPacketSimple(61, player, true), worldObj.provider.getDimension());
				}
	...
	public static boolean checkTargetInShootingView(EntityLivingBase entity, Vec3d target) {
		...
		return EntityHelper.canBeSeen(entity.worldObj, shooting, target);
	}
	...
	public void clearAttackTarget(LivingSetAttackTargetEvent event) {
		if (!event.getEntity().worldObj.isRemote && event.getTarget() != null && 
	...
	public void stopHacking(LivingHurtEvent event) {
		if (!event.getEntity().worldObj.isRemote && TickHandler.hasHandler(event.getEntity(), Identifier.SOMBRA_HACK)) {
			TickHandler.unregister(false, TickHandler.getHandler(event.getEntity(), Identifier.SOMBRA_HACK));
			Minewatch.network.sendToDimension(new SPacketSimple(61, event.getEntity(), false), event.getEntity().worldObj.provider.getDimension());
		}
	}
