...
{	
	...
	public void onUsingTick(ItemStack stack, EntityLivingBase player, int count) {
		if (!player.worldObj.isRemote && player instanceof EntityPlayer) {
			if (player.getHeldItemMainhand() != null && player.getHeldItemMainhand().getItem() instanceof ItemTracerPistol) {
				for (int i=0; i<2; i++)
					player.worldObj.spawnEntityInWorld(new EntityTracerBullet(player.worldObj, player, EnumHand.MAIN_HAND));
				player.worldObj.playSound(null, player.posX, player.posY, player.posZ, ModSoundEvents.tracerPistol, SoundCategory.PLAYERS, 1.0f, player.worldObj.rand.nextFloat()/20+0.95f);	
				if (count == 20 && !ModArmor.isSet((EntityPlayer)player, ModItems.tracer))
					player.getHeldItemMainhand().damageItem(1, player);
			}
			if (player.getHeldItemOffhand() != null && player.getHeldItemOffhand().getItem() instanceof ItemTracerPistol) {
				for (int i=0; i<2; i++)
					player.worldObj.spawnEntityInWorld(new EntityTracerBullet(player.worldObj, player, EnumHand.OFF_HAND));
				player.worldObj.playSound(null, player.posX, player.posY, player.posZ, ModSoundEvents.tracerPistol, SoundCategory.PLAYERS, 1.0f, player.worldObj.rand.nextFloat()/20+0.95f);
				if (count == 20 && !ModArmor.isSet((EntityPlayer)player, ModItems.tracer))
		...
		if (player.worldObj.isRemote) {
			...
			if (player.onGround)
				f6 = player.worldObj.getBlockState(blockpos$pooledmutableblockpos).getBlock().slipperiness * 0.91F;
