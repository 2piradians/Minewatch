	...
	public void onUsingTick(ItemStack stack, EntityLivingBase player, int count) {
		...
		if (!player.worldObj.isRemote && player instanceof EntityPlayer) {
			if (player.getHeldItemMainhand() != null && player.getHeldItemMainhand().getItem() instanceof ItemTracerPistol
				...
				for (int i=0; i<2; i++)
					player.worldObj.spawnEntityInWorld(new EntityTracerBullet(player.worldObj, player));
				player.worldObj.playSound(null, player.posX, player.posY, player.posZ, 
						ModSoundEvents.tracerPistol, SoundCategory.PLAYERS, 1.0f, player.worldObj.rand.nextFloat()/20+0.95f);	
