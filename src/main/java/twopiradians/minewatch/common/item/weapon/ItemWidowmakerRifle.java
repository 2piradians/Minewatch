	...
	public void onItemLeftClick(ItemStack stack, World world, EntityPlayer player, EnumHand hand) { 
		if (this.canUse(player, true)) {
			...
			if (Minewatch.keys.rmb(player) && player.getActiveItemStack() == stack) {
				if (!player.worldObj.isRemote) {
					EntityWidowmakerBullet bullet = new EntityWidowmakerBullet(player.worldObj, player, true, 
					...
					player.worldObj.playSound(null, player.posX, player.posY, player.posZ, ModSoundEvents.widowmakerScopedShoot, SoundCategory.PLAYERS, player.worldObj.rand.nextFloat()+0.5F, player.worldObj.rand.nextFloat()/2+0.75f);	
					...
					Minewatch.proxy.spawnParticlesSpark(player.worldObj, vec.xCoord, vec.yCoord, vec.zCoord, 0xF9394F, 0x5A575A, 5, 1);
					player.stopActiveHand();
				}
			}
			// unscoped
