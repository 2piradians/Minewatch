...
public class ItemWidowmakerRifle extends ItemMWWeapon {
	...
	@Override
	public void onPlayerStoppedUsing(ItemStack stack, World worldObj, EntityLivingBase entity, int timeLeft) {
	...
	public void onUpdate(ItemStack stack, World worldObj, Entity entity, int itemSlot, boolean isSelected) {
		...
		if (isSelected && entity instanceof EntityLivingBase && ((EntityLivingBase)entity).getHeldItemMainhand() == stack) {	
			...
			if (!worldObj.isRemote && hero.ability2.isSelected(player, true) && 
					this.canUse(player, true, EnumHand.MAIN_HAND, true)) {
				EntityWidowmakerHook projectile = new EntityWidowmakerHook(worldObj, player, EnumHand.OFF_HAND.ordinal());
				...
				Minewatch.network.sendToDimension(new SPacketSimple(55, player, false, projectile), worldObj.provider.getDimension());
	...
	public boolean shouldRenderHand(AbstractClientPlayer player, EnumHand hand) {
		return hand == EnumHand.OFF_HAND &&
				TickHandler.hasHandler(handler -> handler.identifier == Identifier.WIDOWMAKER_HOOK && handler.entityLiving == Minecraft.getMinecraft().thePlayer, true);
	}
