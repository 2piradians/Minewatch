...
public class ItemZenyattaWeapon extends ItemMWWeapon {
	...
	public static final Handler HARMONY = new Handler(Identifier.ZENYATTA_HARMONY, false) {
		...
		public boolean onClientTick() {
			...
			if (this.entityLiving != null && this.ticksLeft % 30 == 0) {
				...
				Minewatch.proxy.spawnParticlesCustom(EnumParticle.CIRCLE, this.entityLiving.worldObj, this.entityLiving, 0xCFC77F, 0xCFC77F, 0.3f, 
		...
		public Handler onServerRemove() {
			// stop on client
			Minewatch.network.sendToDimension(new SPacketSimple(42, false, null, this.ticksLeft, 0, 0, this.entity, this.entityLiving), this.entity.worldObj.provider.getDimension());
	...
	public static final Handler DISCORD = new Handler(Identifier.ZENYATTA_DISCORD, false) {
		...
		public boolean onClientTick() {
			...
			if (this.entityLiving != null && this.ticksLeft % 30 == 0) {
				float size = Math.min(entityLiving.height, entityLiving.width)*8f;
				Minewatch.proxy.spawnParticlesCustom(EnumParticle.ANA_HEAL, this.entityLiving.worldObj, this.entityLiving, 0x8D4BF3, 0x8D4BF3, 0.4f, 
						40, size, size, this.entityLiving.worldObj.rand.nextFloat(), this.entityLiving.worldObj.rand.nextFloat()/20f);
			}
			return super.onClientTick() || (entityLiving != null && !entityLiving.isEntityAlive());
		}
		@Override
		public Handler onServerRemove() {
			// stop on client
			Minewatch.network.sendToDimension(new SPacketSimple(43, false, null, this.ticksLeft, 0, 0, this.entity, this.entityLiving), this.entity.worldObj.provider.getDimension());
			return super.onServerRemove();
		}
		@Override
		@SideOnly(Side.CLIENT)
	...
	public static final Handler VOLLEY = new Handler(Identifier.ZENYATTA_VOLLEY, true) {
		...
		public boolean onServerTick() {
			if (entityLiving != null && this.ticksLeft % 2 == 0) {
				EntityZenyattaOrb orb = new EntityZenyattaOrb(entityLiving.worldObj, entityLiving, 2, 0);
				...
				ModSoundEvents.ZENYATTA_VOLLEY_SHOOT.playFollowingSound(entityLiving, entityLiving.worldObj.rand.nextFloat()+0.5F, entityLiving.worldObj.rand.nextFloat()/3+0.8f, false);
			}
	...
	public void onItemLeftClick(ItemStack stack, World worldObj, EntityLivingBase player, EnumHand hand) { 		
		...
		if (this.canUse(player, true, hand, false) && !player.isHandActive() && !TickHandler.hasHandler(player, Identifier.ZENYATTA_VOLLEY)) {
			if (!worldObj.isRemote) {
				EntityZenyattaOrb orb = new EntityZenyattaOrb(worldObj, player, hand.ordinal(), 0);
				...
				ModSoundEvents.ZENYATTA_SHOOT.playFollowingSound(player, worldObj.rand.nextFloat()+0.5F, worldObj.rand.nextFloat()/3+0.8f, false);
	...
	public void onUpdate(ItemStack stack, World worldObj, Entity entity, int slot, boolean isSelected) {
		...
		if (isSelected && entity instanceof EntityLivingBase && ((EntityLivingBase) entity).getHeldItemMainhand() == stack &&
			...
			// harmony
			if (!worldObj.isRemote && hero.ability1.isSelected(player, player instanceof EntityPlayer) && 
				...
				if (target != null) {
					...
					Minewatch.network.sendToDimension(new SPacketSimple(42, player, true, target), worldObj.provider.getDimension());
					...
					// shoot orb
					EntityZenyattaOrb orb = new EntityZenyattaOrb(worldObj, player, EnumHand.OFF_HAND.ordinal(), 2);
			...
			// discord
			if (!worldObj.isRemote && hero.ability2.isSelected(player, true) && 
					this.canUse(player, true, EnumHand.MAIN_HAND, true)) {
				...
				if (target != null) {
					...
					Minewatch.network.sendToDimension(new SPacketSimple(43, player, true, target), worldObj.provider.getDimension());
					...
					// shoot orb
					EntityZenyattaOrb orb = new EntityZenyattaOrb(worldObj, player, EnumHand.MAIN_HAND.ordinal(), 1);
	...
	public void onUsingTick(ItemStack stack, EntityLivingBase player, int count) {
		...
		if (player.worldObj.isRemote) {
			player.rotationPitch += (player.worldObj.rand.nextFloat()-0.5f)*(count-this.getMaxItemUseDuration(stack))*0.008f;
			player.rotationYaw += (player.worldObj.rand.nextFloat()-0.5f)*(count-this.getMaxItemUseDuration(stack))*0.008f;
		}
		// secondary particles
		if (count > 0 && count % VOLLEY_CHARGE_DELAY == 0) {
			...
			if (num <= 5 && num > 0) {
				if (player.worldObj.isRemote)
					Minewatch.proxy.spawnParticlesMuzzle(EnumParticle.ZENYATTA, player.worldObj, player, 0xFFFFFF, 0xFFFFFF, 1, count, 2.5f, 2.9f, player.worldObj.rand.nextFloat(), 0.001f, null, y, x);
				else {
					this.subtractFromCurrentAmmo(player, 1);
	...
	public ItemStack onItemUseFinish(ItemStack stack, World worldObj, EntityLivingBase entity) {
		this.shootVolley(entity, 5);
		return super.onItemUseFinish(stack, worldObj, entity);
	}
	...
	private void shootVolley(EntityLivingBase entity, int orbs) {
		if (!entity.worldObj.isRemote)
			ModSoundEvents.ZENYATTA_VOLLEY_CHARGE.stopSound(entity.worldObj);
		...
		TickHandler.register(entity.worldObj.isRemote, VOLLEY.setEntity(entity).setTicks(orbs*2));
	}
