...
public class PotionFrozen extends Potion {
	...
	public void preventAttacking(LivingAttackEvent event) {
		if (event.getSource().getTrueSource() instanceof EntityLivingBase &&
				((EntityLivingBase) event.getSource().getTrueSource()).getActivePotionEffect(ModPotions.frozen) != null && 
				((EntityLivingBase) event.getSource().getTrueSource()).getActivePotionEffect(ModPotions.frozen).getDuration() > 0) {
			if (event.getEntity() instanceof EntityLiving)
				((EntityLiving)event.getSource().getTrueSource()).setAttackTarget(null);
			((EntityLivingBase) event.getSource().getTrueSource()).setRevengeTarget(null);
			event.setCanceled(true);
		}
	}
