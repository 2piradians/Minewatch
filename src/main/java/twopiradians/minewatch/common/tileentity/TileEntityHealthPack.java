...
public abstract class TileEntityHealthPack extends TileEntity implements ITickable {
	...
	public void update() {		
		if (this.cooldown > 0) {
			...
			if (--this.cooldown % 100 == 0 && this.cooldown < this.getResetCooldown() && !worldObj.isRemote) {
				...
				if (this.cooldown == 0)
					ModSoundEvents.HEALTH_PACK_RESPAWN.playSound(worldObj, pos.getX(), pos.getY(), pos.getZ(), 1.0f, 1.0f);
			}
	...
	public void setResetCooldown() {
		...
		if (!worldObj.isRemote) {
			this.worldObj.markAndNotifyBlock(pos, this.worldObj.getChunkFromBlockCoords(pos), this.getBlockType().getDefaultState(), this.getBlockType().getDefaultState(), 2);
			ModSoundEvents.HEALTH_PACK_USE.playSound(worldObj, pos.getX(), pos.getY(), pos.getZ(), 1.0f, 1.0f);
		}
	}
