...
public class EntityHelper {
	...
	public static RayTraceResult checkForImpact(Entity entityIn, Entity thrower, boolean friendly) {
		...
		Entity entity = null;
		List<Entity> list = entityIn.world.getEntitiesWithinAABBExcludingEntity(entityIn, entityIn.getEntityBoundingBox().grow(entityIn.motionX, entityIn.motionY, entityIn.motionZ).grow(1.0D));
		double d0 = 0.0D;
	...
	public static boolean shouldHit(Entity thrower, Entity entityHit, boolean friendly, DamageSource source) {
		...
		return ((entityHit instanceof EntityLivingBase && ((EntityLivingBase)entityHit).getHealth() > 0) || 
				entityHit instanceof EntityDragon) && thrower != null && entityHit != thrower &&
	...
	public static RayTraceResult getMouseOverEntity(EntityLivingBase shooter, int distance, boolean friendly, float pitch, float yawHead) {
		...
		if (shooter != null) {
			...
			List<Entity> list = shooter.world.getEntitiesInAABBexcluding(shooter, shooter.getEntityBoundingBox().grow(vec3d1.x * d0, vec3d1.y * d0, vec3d1.z * d0).grow(1.0D, 1.0D, 1.0D), Predicates.and(EntitySelectors.NOT_SPECTATING, new Predicate<Entity>() {
