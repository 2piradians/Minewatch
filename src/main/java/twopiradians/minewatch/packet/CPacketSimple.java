			...
			mainThread.addScheduledTask(new Runnable() {
				...
				public void run() {
					...
					// Team Selector set team
						...
						Team team = packetPlayer.worldObj.getScoreboard().getTeam(packet.string);
						for (ItemStack stack : packetPlayer.getHeldEquipment())
					...
					// Team Selector gui set entity team
						...
						try {
							if (packet.string.isEmpty())
								packetPlayer.worldObj.getScoreboard().removePlayerFromTeams(entity instanceof EntityPlayer ? entity.getName() : entity.getCachedUniqueIdString());
							else
								packetPlayer.worldObj.getScoreboard().addPlayerToTeam(entity instanceof EntityPlayer ? entity.getName() : entity.getCachedUniqueIdString(), packet.string);
						}
						catch (Exception e) {}
					}
					...
					// Team Selector delete team
						...
						ScorePlayerTeam team = packetPlayer.worldObj.getScoreboard().getTeam(packet.string);
						if (team != null)
							packetPlayer.worldObj.getScoreboard().removeTeam(team);
					}
					...
					// Team Selector create team
						...
						try {
							ScorePlayerTeam team = packetPlayer.worldObj.getScoreboard().createTeam(packet.string);
							if (team != null) {
								TextFormatting format = TextFormatting.fromColorIndex((int) packet.x);
					...
					// GuiTab select hero
					else if (packet.type == 11 && packetPlayer != null && packetPlayer.worldObj.getMinecraftServer() != null) {
						packetPlayer.worldObj.getMinecraftServer().commandManager.executeCommand(packetPlayer, packet.string);
					}
					// player death screen
					...
					// GuiTeamBlock set team
							...
							packetPlayer.worldObj.getTileEntity(new BlockPos(packet.x, packet.y, packet.z)) instanceof TileEntityTeam) {
						Team team = packetPlayer.worldObj.getScoreboard().getTeam(packet.string);
						TileEntityTeam te = (TileEntityTeam) packetPlayer.worldObj.getTileEntity(new BlockPos(packet.x, packet.y, packet.z));
						te.setTeam(team);
					}
					// GuiTeamBlock set team
					...
					// GuiTeamSpawn increase/decrease spawnRadius
							...
							packetPlayer.worldObj.getTileEntity(new BlockPos(packet.x, packet.y, packet.z)) instanceof TileEntityTeamSpawn) {
						TileEntityTeamSpawn te = (TileEntityTeamSpawn) packetPlayer.worldObj.getTileEntity(new BlockPos(packet.x, packet.y, packet.z));
