		...
		public IMessage onMessage(final SPacketSimple packet, final MessageContext ctx) {
			...
			mainThread.addScheduledTask(new Runnable() {
				...
				public void run() {
					...
					Entity entity2 = packet.id2 == -1 ? null : player.worldObj.getEntityByID(packet.id2);
					...
					// Reaper's teleport
						...
						Minewatch.proxy.spawnParticlesReaperTeleport(entity.worldObj, (EntityLivingBase) entity, true, 0);
						Minewatch.proxy.spawnParticlesReaperTeleport(entity.worldObj, (EntityLivingBase) entity, false, 0);
						if (player == entity)
					...
					// Damage entity
						...
						// play damage sound
						ModSoundEvents.HURT.playSound(player, (float) MathHelper.clamp_double(packet.x/18f, 0.1f, 0.4f), 1.0f, true);
					}
					// Interrupt
					...
					// Junkrat's grenade bounce
						...
						// bounce
							...
							Minewatch.proxy.spawnParticlesCustom(EnumParticle.SPARK, entity.worldObj, entity.posX, entity.posY, entity.posZ,
									0, 0, 0, 0xFCCD75, 0xFFDA93, 0.7f, 2, 3, 2.5f, entity.worldObj.rand.nextFloat(), 0.01f);
					...
					// Frozen particles
						...
						for (int i=0; i<3; ++i)
							Minewatch.proxy.spawnParticlesCustom(EnumParticle.CIRCLE, entity.worldObj, 
									packet.x+entity.worldObj.rand.nextDouble()-0.5d, 
									packet.y+entity.worldObj.rand.nextDouble()-0.5d, 
									packet.z+entity.worldObj.rand.nextDouble()-0.5d, 
									0, 0.01f, 0, 0x5BC8E0, 0xAED4FF,
									entity.worldObj.rand.nextFloat(), 5, 20f, 25f, 0, 0);
					}
					...
					// Junkrat trap
						...
						if (packetPlayer == player && player != null)
							Minewatch.proxy.spawnParticlesCustom(EnumParticle.JUNKRAT_TRAP_TRIGGERED, player.worldObj, entity.posX, entity.posY+1.5d, entity.posZ, 0, 0, 0, 0xFFFFFF, 0xFFFFFF, 1, 80, 5, 5, 0, 0);
					}
						...
						if (packet.bool)
							for (int i=0; i<30; ++i)
								entity.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, 
										entity.posX+(entity.worldObj.rand.nextDouble()-0.5d)*1d, entity.posY+entity.worldObj.rand.nextDouble()*1d, entity.posZ+(entity.worldObj.rand.nextDouble()-0.5d)*1d, 0, 0, 0, new int[0]);
						else
					...
					// Lucio's soundwave
						...
						Minewatch.proxy.spawnParticlesMuzzle(EnumParticle.CIRCLE, entity.worldObj, (EntityLivingBase) entity, 
						...
						Minewatch.proxy.spawnParticlesMuzzle(EnumParticle.HOLLOW_CIRCLE, entity.worldObj, (EntityLivingBase) entity, 
								0xDFED8B, 0xABBF78, 0.7f, 15, 5+(entity.worldObj.rand.nextFloat()-0.5f)*2f, 7+(entity.worldObj.rand.nextFloat()-0.5f)*2f, entity.worldObj.rand.nextFloat(), entity.worldObj.rand.nextFloat()/10f, EnumHand.MAIN_HAND, 17, 0.65f);
						Minewatch.proxy.spawnParticlesMuzzle(EnumParticle.HOLLOW_CIRCLE, entity.worldObj, (EntityLivingBase) entity, 
								0xDFED8B, 0xABBF78, 0.7f, 14, 2+(entity.worldObj.rand.nextFloat()-0.5f)*2f, 6+(entity.worldObj.rand.nextFloat()-0.5f)*2f, entity.worldObj.rand.nextFloat(), entity.worldObj.rand.nextFloat()/10f, EnumHand.MAIN_HAND, 17, 0.65f);
						Minewatch.proxy.spawnParticlesMuzzle(EnumParticle.HOLLOW_CIRCLE, entity.worldObj, (EntityLivingBase) entity, 
								0xDFED8B, 0xABBF78, 0.7f, 13, 3+(entity.worldObj.rand.nextFloat()-0.5f)*2f, 6+(entity.worldObj.rand.nextFloat()-0.5f)*2f, entity.worldObj.rand.nextFloat(), entity.worldObj.rand.nextFloat()/10f, EnumHand.MAIN_HAND, 17, 0.65f);
					...
					// Hurt sound for Mercy's power beam
						...
						ModSoundEvents.HURT.playSound(entity, 0.3f, entity.worldObj.rand.nextFloat()/2+0.75f, true);
					}
					...
					// Zenyatta's Harmony (entity is zen, entity2 is target)
						...
						if (packet.bool) {
							...
							if (handler != null) 
								...
								Minewatch.proxy.spawnParticlesCustom(EnumParticle.ZENYATTA_HARMONY_ORB, entity.worldObj, entity2, 0xFFFFFF, 0xFFFFFF, 1.0f, Integer.MAX_VALUE, 3, 3, 0, 0);
							...
							if (entity == player) {
								...
								Minewatch.proxy.spawnParticlesCustom(EnumParticle.ZENYATTA_HARMONY, entity.worldObj, entity2, 0xFFFFFF, 0xFFFFFF, 1.0f, Integer.MAX_VALUE, 1, 1, 0, 0);
							}
						}
						// end
						else {
					...
					// Zenyatta's Discord (entity is zen, entity2 is target)
						...
						if (packet.bool) {
							...
							if (handler != null) 
								...
								Minewatch.proxy.spawnParticlesCustom(EnumParticle.ZENYATTA_DISCORD_ORB, entity.worldObj, entity2, 0xFFFFFF, 0xFFFFFF, 1.0f, Integer.MAX_VALUE, 3, 3, 0, 0);
							...
							if (entity == player) {
								...
								Minewatch.proxy.spawnParticlesCustom(EnumParticle.ZENYATTA_DISCORD, entity.worldObj, entity2, 0xFFFFFF, 0xFFFFFF, 1.0f, Integer.MAX_VALUE, 1, 1, 0, 0);
							}
						}
						// end
						else {
					...
					// McCree's flashbang
						...
						Minewatch.proxy.spawnParticlesCustom(EnumParticle.STUN, entity.worldObj, entity, 0xFFFFFF, 0xFFFFFF, 0.9f, 14, size, size, 0, 0);
					...
					// Ana's grenade
						...
						if (packet.bool) {
							Minewatch.proxy.spawnParticlesCustom(EnumParticle.ANA_GRENADE_HEAL, entity.worldObj, entity, 0xFFFFFF, 0xFFFFFF, 1, 80, 2, 2, 0, 0);
							...
							Minewatch.proxy.spawnParticlesCustom(EnumParticle.ANA_GRENADE_DAMAGE, entity.worldObj, entity, 0xFFFFFF, 0xFFFFFF, 1, 80, 2, 2, 0, 0);
					...
					// Tracer's recall
						...
						Minewatch.proxy.spawnParticlesCustom(EnumParticle.HOLLOW_CIRCLE_3, entity.worldObj, entity.posX, entity.posY+entity.height/2f, entity.posZ, 0, 0, 0, 0x63B8E8, 0x4478AD, 1, 7, 20, 0, 0, 0.5f);
						Minewatch.proxy.spawnParticlesCustom(EnumParticle.HOLLOW_CIRCLE_2, entity.worldObj, entity.posX, entity.posY+entity.height/2f, entity.posZ, 0, 0, 0, 0x63B8E8, 0x4478AD, 1, 7, 20, 0, 0, 0.5f);
						...
						Minewatch.proxy.spawnParticlesCustom(EnumParticle.SPARK, entity.worldObj, entity.posX, entity.posY+entity.height/2f, entity.posZ, 0, 0, 0, 0xD2FFFF, 0xEAFFFF, 1, 15, 0, 10, 0, 0.1f);
					...
					// Reinhardt's charge
						...
						// unregister charge
							...
							if (result != null) {
								...
								Minewatch.proxy.spawnParticlesCustom(EnumParticle.REINHARDT_CHARGE, entity.worldObj, pos.xCoord, pos.yCoord, pos.zCoord, 0, 0, 0, 0xFFFFFF, 0xFFFFFF, 1.0f, 300, 10, 9, entity.worldObj.rand.nextFloat(), 0, result.sideHit, true);
							}
							// unregister
					...
					// Set player's rotations
						...
						//player.rotationYaw = (float) MathHelper.wrapDegrees(MathHelper.clamp_double(MathHelper.wrapDegrees(packet.x), MathHelper.wrapDegrees(player.rotationYaw-1), MathHelper.wrapDegrees(player.rotationYaw+1)));
						//player.rotationPitch = (float) MathHelper.wrapDegrees(MathHelper.clamp_double(MathHelper.wrapDegrees(packet.y), MathHelper.wrapDegrees(player.rotationPitch-1), MathHelper.wrapDegrees(player.rotationPitch+1)));
					}
					// Ranks
					...
					// Doomfist's punch
						...
						if (packet.bool) {
							...
							if (entity2 instanceof EntityLivingBase) {
								...
								Minewatch.proxy.spawnParticlesCustom(EnumParticle.CIRCLE, entity.worldObj, vec.xCoord, vec.yCoord, vec.zCoord, 0, 0, 0, 0xFFD9B2, 0xE8C4A2, 1f, 4, 10, 15, 0, 0);
								Minewatch.proxy.spawnParticlesCustom(EnumParticle.DOOMFIST_PUNCH_3, entity.worldObj, vec.xCoord, vec.yCoord, vec.zCoord, 0, 0, 0, 0xFFFFFF, 0xFFFFFF, 0.7f, 4, 10, 15, 0, 0);
							}
					...
					// doomfist charge punch
						...
						Minewatch.proxy.spawnParticlesMuzzle(EnumParticle.DOOMFIST_PUNCH_3, entity.worldObj, (EntityLivingBase) entity, 0xFFFFFF, 0xDDDDFF, 1, EnumHero.DOOMFIST.weapon.getMaxItemUseDuration(((EntityLivingBase)entity).getHeldItemMainhand())+8, 4, 6, entity.worldObj.rand.nextFloat()*2f, 0.01f, EnumHand.MAIN_HAND, 5, 1);
						((EntityLivingBase) entity).setActiveHand(EnumHand.MAIN_HAND);
					}
					// doomfist slam
					else if (packet.type == 67 && entity != null) {
						Minewatch.proxy.spawnParticlesCustom(EnumParticle.DOOMFIST_SLAM_1, entity.worldObj, packet.x, packet.y, packet.z, 0, 0, 0, 0xFFFFFF, 0xFFFFFF, 0.8f, 100, 60, 60, (entity.rotationYaw + 90f) / 180f, 0, EnumFacing.UP, false);
						Minewatch.proxy.spawnParticlesCustom(EnumParticle.DOOMFIST_SLAM_2, entity.worldObj, packet.x, packet.y, packet.z, 0, 0, 0, 0x90FFF9, 0x90FFF9, 0.5f, 10, 60, 60, (entity.rotationYaw + 90f) / 180f, 0, EnumFacing.UP, false);
					}
				}
			});
			return null;
